...
if (isset($_POST['forgot'])) {
    ...
    if (empty($email)) {
        header('location: ../forgot.php?id=' . $usertype . '&e=Please fill up the filled!');
        exit();
    }
    if ($sql->rowCount() == 0) {
        header('location: ../forgot.php?id=' . $usertype . '&e=The account does not exist!');
        exit();
    }

    $sql = $dbh->prepare('UPDATE user SET token = :token WHERE email = :email');
    $sql->execute(['email' => $email, 'token' => $token]);

    $mail = new PHPmailer();
    header('location: ../forgot.php?id=' . $usertype . '&m=We have sent you an email for the instruction on resetting your password.');
                ...
                <div style='width: 90%; background-color: #fff; margin: auto;'>
                    <div style='width: 90%; background-color: transparent; margin: auto;'>
                        <div style='width: 100%; background-color: #4ab4f4; margin: auto;'>
                            <br/><h1 style='text-align: center; color: #fff;'><strong>Forgot Password</strong></h1><br/>
                        </div>
                    
                        <p>Hi " . $user['firstname'] . ",</p>
                        
                        <p style='text-align: justify;'>To change your password please click the link below.</p>
                        
                        <p style='text-align: justify;'><a href='http://localhost/hulam/changepassword.php?token=" . $token . "'>http://localhost/hulam/changepassword.php?token=" . $token . "</a></p>
