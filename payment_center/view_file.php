                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            ...
                                            if ($query->rowCount() > 0) {
                                                foreach ($res as $rem) {
                                                   $sed = htmlentities($rem->file_status);
                                                    ...
                                                    ?>
                                            <tr>
                                                <td><?= htmlentities($rem->filedate_upload)?></td>
                                                <td><?= htmlentities($rem->files)?></td>
                                                <td><?php if($sed == "Sent"): ?>
                                            ...
                                            <?php $cnt = $cnt+1; }}}
                                                ...
                                                $user_id = $_SESSION['user_id'];
                                                $sql = "SELECT * FROM uploadfile_payment WHERE uploader_id LIKE '%$search_input%' OR files LIKE '%$search_input%' OR filedate_upload LIKE '%$search_input%' OR file_status LIKE '%$search_input%' AND (lender_id = $lender_id AND uploader_id = $user_id) ORDER BY filedate_upload desc";
                                                $query = $dbh->prepare($sql);
                                                $query->execute();
                                                $res = $query->fetchAll(PDO::FETCH_OBJ);
                                                $cnt =1; 
