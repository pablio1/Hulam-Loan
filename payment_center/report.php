								...
								<div class="card-body">
									...
									<table class="table table-bordered">
                                        <thead>
                                            <tr>
												<th>ID</th>
                                                <th>Individual/Lending Company</th>
                                        ...
                                        <?php
                                            ...
                                            $sql = "SELECT * FROM uploadfile_payment INNER JOIN user ON uploadfile_payment.uploader_id = user.user_id WHERE uploader_id = $user_id ORDER BY filedate_upload desc";
                                            $query = $dbh->prepare($sql);
                                            $query->execute();
											...
											$user_id = $_SESSION['user_id'];
											$sql = "SELECT * FROM uploadfile_payment INNER JOIN user ON uploadfile_payment.lender_id = user.user_id WHERE (user.company_name LIKE '%$search_input%' OR uploadfile_payment.file_status LIKE '%$search_input%' OR uploadfile_payment.files LIKE '%$search_input%' OR user.email LIKE '%$search_input%' OR user.lastname LIKE '%$search_input%' 
											OR user.firstname LIKE '%$search_input%' OR user.middlename LIKE '%$search_input%' OR user.user_id LIKE '%$search_input%') AND uploader_id = $user_id ORDER BY filedate_upload desc";
                                            $query = $dbh->prepare($sql);
                                            $query->execute();
                                            $res = $query->fetchAll(PDO::FETCH_OBJ);
											$cnt=1;
											...
											<?php $cnt = $cnt+1; }}}
												...
												$sql = "SELECT * FROM uploadfile_payment INNER JOIN user ON uploadfile_payment.lender_id = user.user_id WHERE uploader_id = $user_id ORDER BY filedate_upload desc";
												$query = $dbh->prepare($sql);
												$query->execute();
												$res = $query->fetchAll(PDO::FETCH_OBJ);
												$cnt=1;
												...
												<tr>
													<td><?= htmlentities($rem->user_id)?></td>
													<td><?php if($type=='3'): ?><?= htmlentities($rem->company_name); ?>
														<?php else:?><?= htmlentities($rem->firstname); ?>&nbsp;<?= htmlentities($rem->middlename); ?>&nbsp;<?= htmlentities($rem->lastname); ?>
														<?php endif; ?></td>
													<td><?= htmlentities($rem->files)?></td>
													<td><?= htmlentities($rem->filedate_upload)?></td>
													<td><?php if($sed == "Sent"): ?>
